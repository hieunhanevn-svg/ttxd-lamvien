...
// Ghi vào hàng cụ thể - không dùng append để tránh lỗi vị trí
async function sheetsWriteRow(sheetName, values) {
  // Đọc cột A để tìm hàng tiếp theo thực sự có dữ liệu
  const colA = await sheetsGet(`${sheetName}!A:A`);
  // Tìm hàng cuối cùng có dữ liệu (bỏ qua hàng trống)
  let lastRow = 1; // Bắt đầu từ header row
  for (let i = 1; i < colA.length; i++) {
    if (colA[i] && colA[i][0] && colA[i][0].toString().trim()) {
      lastRow = i + 1; // +1 vì sheet rows 1-based
    }
  }
  const newRow = lastRow + 1;
  await sheetsUpdate(`${sheetName}!A${newRow}:${String.fromCharCode(64 + values.length)}${newRow}`, [values]);
  return newRow;
}
