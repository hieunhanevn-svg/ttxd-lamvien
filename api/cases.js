...
const LAT_MIN=11.8, LAT_MAX=12.1, LNG_MIN=108.3, LNG_MAX=108.6;
...
function validateCoordBackend(lat, lng) {
  const la = parseCoord(lat), lo = parseCoord(lng);
  if (la!==''&&(la<LAT_MIN||la>LAT_MAX)) return 'Vi do '+la+' ngoai khoang hop le Da Lat ('+LAT_MIN+'-'+LAT_MAX+')';
  if (lo!==''&&(lo<LNG_MIN||lo>LNG_MAX)) return 'Kinh do '+lo+' ngoai khoang hop le Da Lat ('+LNG_MIN+'-'+LNG_MAX+')';
  return null;
}
...
async function nextMaHoSo() {
  ...
  // Đọc tất cả sheet LICHSU_[năm] đang tồn tại để tránh trùng mã với hồ sơ đã archive
  const sheetIds = await getSheetIds();
  const lichsuSheets = Object.keys(sheetIds).filter(n => n.startsWith('LICHSU_'));

  // Batch query HO_SO + tất cả LICHSU cùng 1 request
  const ranges = ['HO_SO!A2:A', ...lichsuSheets.map(s => s + '!A2:A')];
  const results = await sheetsBatchGet(ranges);

  // Gộp tất cả mã hồ sơ từ mọi sheet
  const allCodes = results.flat(2).map(v => String(v||'').trim()).filter(Boolean);

  // Tìm số lớn nhất trong năm hiện tại
  let maxNum = 0;
  for (const code of allCodes) {
    if (code.startsWith(prefix)) {
      const n = parseInt(code.replace(prefix, ''));
      if (!isNaN(n) && n > maxNum) maxNum = n;
    }
  }

  // Tăng thêm 1 + double-check không trùng
  let num = maxNum + 1;
  let maHoSo = prefix + String(num).padStart(3, '0');
  while (allCodes.includes(maHoSo)) {
    num++;
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'addCase') {
      ...
      const coordErr = validateCoordBackend(toa_do_lat, toa_do_lng);
      if (coordErr) return res.json({ success:false, error:coordErr });
      ...
      await sheetsWriteRow('HO_SO', [
        ...
        parseCoord(toa_do_lat), parseCoord(toa_do_lng),
    ...
    if (action === 'updateCase') {
      const { ma_ho_so, chu_dau_tu, dia_chi_chi_tiet, toa_do_lat, toa_do_lng,
              loai_vi_pham, bien_phap, deadline, ghi_chu } = body;
      if (!ma_ho_so) return res.json({ success:false, error:'Thieu ma ho so' });

      // Lấy hồ sơ hiện tại
      const hsRows = await sheetsGet('HO_SO!A2:R');
      const idx = hsRows.findIndex(r => r[0] === ma_ho_so);
      if (idx < 0) return res.json({ success:false, error:'Khong tim thay ho so' });

      const hs = hsRows[idx];
      // Kiểm tra quyền
      if (user.role !== 'admin' && user.username !== hs[2]) {
        return res.json({ success:false, error:'Ban khong co quyen sua ho so nay' });
      }
      // Không cho sửa hồ sơ đã hoàn thành
      if (hs[13] === 'hoan_thanh') {
        return res.json({ success:false, error:'Khong the sua ho so da hoan thanh' });
      }
      ...
      const rowNum = idx + 2;
      // Ghi các trường được phép sửa
      await sheetsUpdate('HO_SO!D'+rowNum, [[dia_chi_chi_tiet||hs[3]]]);           // D: dia_chi
      await sheetsUpdate('HO_SO!F'+rowNum+':G'+rowNum, [[                           // F-G: toa do
        parseCoord(toa_do_lat)||hs[5],
        parseCoord(toa_do_lng)||hs[6]
      ]]);
      await sheetsUpdate('HO_SO!J'+rowNum, [[chu_dau_tu||hs[9]]]);                  // J: chu_dau_tu
      await sheetsUpdate('HO_SO!K'+rowNum, [[loai_vi_pham||hs[10]]]);               // K: loai_vi_pham
      await sheetsUpdate('HO_SO!L'+rowNum, [[String(bien_phap||hs[11])]]);          // L: bien_phap
      await sheetsUpdate('HO_SO!M'+rowNum, [[deadline||'']]);                       // M: deadline
      await sheetsUpdate('HO_SO!R'+rowNum, [[ghi_chu||'']]);                        // R: ghi_chu

      // Ghi nhật ký tóm tắt
      const changed = [];
      if (dia_chi_chi_tiet && dia_chi_chi_tiet !== hs[3]) changed.push('Dia chi');
      if (chu_dau_tu && chu_dau_tu !== hs[9]) changed.push('Chu DT');
      if (loai_vi_pham && loai_vi_pham !== hs[10]) changed.push('Loai VP');
      if (String(bien_phap) !== String(hs[11])) changed.push('Phuong an');
      if (deadline !== (hs[12]||'')) changed.push('Han XL');
      if (ghi_chu !== (hs[17]||'')) changed.push('Ghi chu');
      if (parseCoord(toa_do_lat) && parseCoord(toa_do_lat) !== parseCoord(hs[5])) changed.push('Toa do');
      const summary = changed.length > 0
        ? 'Da cap nhat thong tin: ' + changed.join(', ')
        : 'Da cap nhat thong tin ho so';

      await sheetsWriteRow('NHAT_KY', [
        'NK-'+Date.now(), ma_ho_so, new Date().toISOString(),
        user.username, 'cap_nhat', summary, '', ''
      ]);

      return res.json({ success:true, message: summary });
    }
