const { sheetsGet, sheetsAppend, sheetsUpdate, sheetsBatchGet, driveCreateFolder, verifyToken, cors, today, formatDate, DRIVE_FOLDER } = require('./_sheets');
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'addCase') {
      ...
      // Tạo folder Drive
      const folderName = `${maHoSo}_${dia_chi_chi_tiet.replace(/[^a-zA-Z0-9àáảãạăắặẳẵằâấầẩẫậèéẻẽẹêếềểễệìíỉĩịòóỏõọôốồổỗộơớờởỡợùúủũụưứừửữựỳýỷỹỵđÀÁẢÃẠĂẮẶẲẴẰÂẤẦẨẪẬÈÉẺẼẸÊẾỀỂỄỆÌÍỈĨỊÒÓỎÕỌÔỐỒỔỖỘƠỚỜỞỠỢÙÚỦŨỤƯỨỪỬỮỰỲÝỶỸỴĐ _-]/g,'')}_${today()}`.substring(0,60);
      let folderId = '';
    ...
    if (action === 'deleteCase') {
      ...
      const { ma_ho_so } = body;
      // Xóa bằng cách xóa toàn bộ dòng (dùng Sheets API batchUpdate)
      const token_g = await (require('./_sheets').getAccessToken ? require('./_sheets').getAccessToken() : null);
      const rows = await sheetsGet('HO_SO!A2:A');
      const idx = rows.findIndex(r => r[0] === ma_ho_so);
      if (idx < 0) return res.json({ success:false, error:'Không tìm thấy hồ sơ' });
      // Xóa nhật ký
      const nkRows = await sheetsGet('NHAT_KY!A2:B');
      const nkIdxs = nkRows.map((r,i)=>r[1]===ma_ho_so?i+1:null).filter(i=>i!==null);
      // Xóa hồ sơ - ghi đè dòng trống
      await sheetsUpdate(`HO_SO!A${idx+2}:R${idx+2}`, [Array(18).fill('')]);
      return res.json({ success:true, message:`Đã xóa hồ sơ ${ma_ho_so}` });
    }
